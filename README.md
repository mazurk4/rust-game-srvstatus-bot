...
`bot/maintenance.txt` を作成するとメンテナンス中として Discord に `🛠️ Maintenance` が表示されます。  
ファイルを削除すると通常のステータス表示に戻ります。

```bash
# メンテナンス開始
touch bot/maintenance.txt

# メンテナンス終了
rm bot/maintenance.txt
```

Docker で運用している場合は `docker exec` で操作します。
